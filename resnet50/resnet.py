...
def parse_args():
    ...
    parser.add_argument(
	'--collect_memory_interval',
	type=int,
        default=10,
	help='the time interval of collect GPU memory data')
...
def run_benchmark(model, args):
	...
	if pass_id == args.pass_num - 1:
	    train_acc_factor.add_record(np.array(pass_train_acc, dtype='float32'))
	...
	train_speed_factor.add_record(np.array(examples_per_sec, dtype='float32'))
...
def collect_gpu_memory_data(time_interval, mem_list):
    """
    ...
    while(True):
    	...
    	if status == 0:
	    memory_data = output.split("\n")[1].split(" ")[0]
            mem_list.append(int(memory_data))
        else:
...
def save_gpu_data(mem_list):
    """
    save the gpu data to a file
    """
    gpu_memory_factor.add_record(mem_list[-1])
...
if __name__ == '__main__':
    ...
    if args.device == 'GPU':
        collect_memory_thread = threading.Thread(target=collect_gpu_memory_data,args=(args.collect_memory_interval,mem_usage_list))
